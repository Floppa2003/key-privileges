...
const cfg=__EKP_CONFIG__, started=Date.now(), deadline=started+43000;
...
function cards(){const out=new Map();for(const a of document.querySelectorAll('main a[href]')){
 const url=cardURL(a.href),n=a.closest('.v-card'),title=n?.querySelector('.v-card-title')?.textContent.trim();
 if(!url||!n||!title||a.textContent.trim()!=='Подробнее')continue;
 const v={url,title,loginNotice:n.textContent.includes('Требуется авторизация'),preview:n.textContent.trim()};
 if(out.has(url)&&JSON.stringify(out.get(url))!==JSON.stringify(v))throw Error('conflicting_catalogue_card');out.set(url,v);
 }return [...out.values()];}
function more(){return [...document.querySelectorAll('main button')].filter(n=>visible(n)&&!n.disabled&&n.textContent.trim()==='Показать еще');}
...
function restricted(){return /access denied|captcha|проверка безопасности|доступ к сайту временно ограничен/i.test(document.title+' '+document.body.textContent.slice(0,800));}
...
async function grow(button,label){const before=cards().length;button.click();await frame();
 ...
 const after=cards().length;result.actions.push({action:label,before,after,elapsedMs:Date.now()-started});if(after<=before)throw Error('catalogue_no_growth');}
try{
 ...
 const sizes=[...document.querySelectorAll('main button')].filter(n=>visible(n)&&!n.disabled&&n.textContent.trim()==='120');
 ...
 result.paginationText=document.querySelector('main .v-pagination')?.textContent||null;
 result.filterText=[...document.querySelectorAll('main .v-select__selection-text')].map(n=>n.textContent.trim());
 result.apiBeforeDetails=apiCount();result.catalogueElapsedMs=Date.now()-started;
 const linksByURL=new Map([...document.querySelectorAll('main a[href]')].filter(n=>n.textContent.trim()==='Подробнее').map(n=>[cardURL(n.href),n]));
 ...
 for(const entry of selected){
  ...
  const a=linksByURL.get(entry.url);
  if(!a?.isConnected)throw Error('detail_link_missing');a.click();await frame();
  ...
  while(Date.now()<until){n=document.getElementById('partner.'+id);if(n&&cardURL(location.href)===entry.url&&
    (n.textContent.includes('Программа лояльности')||n.textContent.includes('Для просмотра подробной информации о программе лояльности авторизуйтесь')))break;await sleep(20);}
  if(!n||cardURL(location.href)!==entry.url){result.errors.push({url:entry.url,reason:'detail_not_ready'});break;}
  const title=n.querySelector('.v-card-title')?.textContent.trim();if(title!==entry.title)throw Error('detail_title_changed');
