...
def run_tool(path,*args):
    p=subprocess.run([str(path),*args],stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=90,text=True)
...
def main():
    ...
    link=download_link(meta)
    raw=request('GET',link,maximum=160_000_000,headers={'User-Agent':HEADERS['User-Agent'],'Accept':'*/*'})
    REPORT['download_sha256']=hashlib.sha256(raw).hexdigest()
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        if sum(i.file_size for i in z.infolist())>400_000_000: raise RuntimeError('expanded_package_bound')
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        REPORT['framework_hints']=[i.filename for i in z.infolist() if re.search(r'index.android.bundle|flutter_assets|assemblies/|assets/www/.*(?:html|js)$',i.filename)][:50]
        for i in z.infolist():
