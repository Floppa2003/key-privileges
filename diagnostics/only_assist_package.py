...
Independent diagnostic; never uses bank/source credentials or publishes app code.
RuStore client flow provenance: Dynamic-Mobile-Security/mdast-cli,
mdast_cli/distribution_systems/rustore.py (read 2026-09-20).
"""
...
def request(method, url, payload=None, maximum=2_000_000, headers=None):
    for _ in range(4):
        ...
        # Signed CDN query values and arbitrary response bodies never enter the report.
        u = urlsplit(url)
        with SESSION.request(method, url, json=payload, headers=headers or HEADERS,
            ...
            if r.status_code in (301, 302, 303, 307, 308):
                if method != 'GET':
                    raise RuntimeError('post_redirect_not_followed')
            ...
            for part in r.iter_content(65536):
                raw.extend(part)
                if len(raw) > maximum:
                    raise RuntimeError('response_size_bound')
            return bytes(raw)
...
def run_tool(path, *args):
    ...
    if p.returncode:
        raise RuntimeError('android_tool_failed:' + Path(path).name)
    return p.stdout
...
def main():
    ...
    obj = json.loads(raw)
    meta = obj.get('body', {})
    if meta.get('packageName') != PACKAGE:
        raise RuntimeError('store_package_mismatch')
    ...
    result = json.loads(request('POST', 'https://backapi.rustore.ru/applicationData/download-link',
                               {'appId': meta['appId'], 'firstInstall': True}))
    link = result.get('body', {}).get('apkUrl')
    if not isinstance(link, str):
        raise RuntimeError('store_download_link_absent')
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        if 'AndroidManifest.xml' not in z.namelist():
            ...
            if len(apks) != 1:
                raise RuntimeError('split_package_requires_explicit_selection')
            raw = z.read(apks[0])
    dest = Path(os.environ.get('RUNNER_TEMP', '/tmp')) / 'only-assist.apk'
    dest.write_bytes(raw)
    REPORT['apk_sha256'] = hashlib.sha256(raw).hexdigest()
    REPORT['apk_bytes'] = len(raw)
    sdk = Path(os.environ['ANDROID_HOME'])
    tools = sorted(sdk.glob('build-tools/*/apksigner'))
    if not tools:
        raise RuntimeError('apksigner_missing')
    ...
    if not pkg or pkg[1] != PACKAGE:
        raise RuntimeError('binary_package_identity_mismatch')
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        for i in z.infolist():
            if i.file_size > 65_000_000 or not (i.filename.endswith(('.dex', '.so', '.js', '.json', '.xml', '.bundle'))):
                continue
            data = z.read(i)
    ...
    # The APK stays in the disposable runner only, not in public artifacts.
    if os.environ.get('GITHUB_OUTPUT'):
        with open(os.environ['GITHUB_OUTPUT'], 'a') as f:
            f.write('apk_ready=true\n')
