...
function snapshot(label){
 ...
 for(const n of copy.querySelectorAll('script,style,noscript,svg,img,form,input,textarea,iframe,[hidden],[aria-hidden="true"]'))n.remove();
 ...
 if(copy.outerHTML.length>4500000)throw new Error('snapshot_limit');
...
try{
 ...
 for(let step=0;step<12&&Date.now()<limit-4000;step++){
  const more=[...document.querySelectorAll('main button')].filter(n=>visible(n)&&!n.disabled&&n.innerText.trim()==='Показать еще');
  if(more.length===0){result.stopReason='visible_load_more_absent';break;}
  if(more.length!==1){result.stopReason='ambiguous_load_more';break;}
  await sleep(1100);const before=cards().length;more[0].click();await sleep(1100);
  const until=Math.min(limit-1000,Date.now()+5000);
  while(cards().length<=before&&Date.now()<until&&!restricted())await sleep(300);
  result.actions.push({action:'load_more',before,after:cards().length});
  if(restricted())throw new Error('restriction_document');
  if(cards().length<=before){result.stopReason='no_card_growth';break;}
 }
 snapshot('expanded');
 result.remainingMore=[...document.querySelectorAll('main button')].filter(n=>visible(n)&&!n.disabled&&n.innerText.trim()==='Показать еще').length;
 result.stopReason=result.stopReason||'time_or_action_limit';
