...
def blocks(raw):
    ...
    for card in cards:
        ...
        result.append({'anchor':anchor,'name':name,'category':text(headings[0].get_text(' ',strip=True)),
                       'summary':summary,'body':body,'links':links,'detail_state':state})
    return result
...
def parse_catalog(raw,observed_at):
    ...
    for block in blocks(raw):
        ...
        rows.append(make_offer('hse_alumni','anchor:'+block['anchor'],PROGRAM,block['name'],benefit,
            ROOT+'#'+block['anchor'] if full else ROOT,observed_at,title=block['name'],category=block['category'],
            conditions=block['body'] if full else block['summary'],redemption=block['body'] if full else '',
            link_kind='page_anchor' if full else 'page_block',locator='a[name="'+block['anchor']+'"] and following sections before next named anchor' if full else '.fa-person__name[href ends with #'+block['anchor']+']',
...
def validate_record(r):
    ...
    if (not re.fullmatch('[A-Za-z0-9_-]{1,100}',b.get('anchor','')) or r['native_id']!='anchor:'+b['anchor']
        ...
        or r['source_url']!=(ROOT+'#'+b['anchor'] if full else ROOT)
        or r['benefit_url']!=(ROOT+'#'+b['anchor'] if full else None)
        or r['link_kind']!=('page_anchor' if full else 'page_block')
