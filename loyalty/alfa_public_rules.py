...
def parse_core(data: bytes, observed_at: str) -> list[dict]:
    ...
    taxi_conditions = _must(
        text,
        r"6\.1\.1\..{0,1400}?Операция\s*становится\s*доступной\s*для\s*получения\s*Выплаты\s*в\s*течение\s*2\s*рабочих\s*дней.{0,300}?лимитам\s*Поощрения\s*того\s*месяца,\s*в\s*котором\s*была\s*оформлена\s*заявка",
        "alfa_taxi_conditions_missing",
    )
